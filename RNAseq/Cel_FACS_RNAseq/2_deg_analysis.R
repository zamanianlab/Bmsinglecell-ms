...
counts.raw <- readRDS("data/counts.raw.rds") 
...
df <- bind_rows(as_tibble(log2(counts(dds, normalized = TRUE)[, 1:3]+1)) %>% 
                  mutate(transformation = "log2(x + 1)"),
                as_tibble(assay(vst)[,1:3]) %>%  mutate(transformation = "vst"))
...
# read in counts (raw) and filter for samples
counts.tpm <- readRDS("data/counts.tpm.rds") 
...
saveRDS(counts, "~/Library/CloudStorage/Box-Box/ZamanianLab/LabMembers/Clair/project_singlecell/BmSinglecell-ms/6_Figures/Figure_3/cel_facs_counts.RDS")

# rename samples
counts$sample <- factor(counts$sample, levels = c( "NegCells", "Small", "Large"), labels = c("NegCells", "Small", "Large"))
...
heatmap <- ggplot(counts, aes(gene_id, sample)) +
  ...
  scale_fill_viridis() +
  labs(x = "*C. elegans* Genes", y = "Sample") +
  theme(
      axis.ticks = element_blank(),
      axis.text.x = element_blank(),
      axis.text.y = element_markdown(),
      axis.title.x = element_markdown(size = 12, angle = 360),
...
data2 <- data %>% 
  ...
  left_join(counts2) %>% 
  subset(counts >= 1) %>% 
  subset(nCount_RNA <= 1800) %>% 
  subset(gene_id == genes)

data2 %>% 
  ggplot(aes(x = UMAP_1, y = UMAP_2))+
  geom_point(data= data, size = 0.5, alpha = 0.1, color = "grey")+
  geom_point(aes(color = counts), size = 0.5)+
  #scale_color_gradient(low = "#4b134f", high = "#ff1515")+
